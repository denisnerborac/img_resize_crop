...
list($source_width, $source_height, $source_type) = getimagesize($sourcepath);
if ($percent > 0) {
$width = $source_width * ($percent / 100);
$height = $source_height * ($percent / 100);
} else $min_dimension = min($source_width, $source_height);
$max_dimension = max($source_height, $source_height);
...
$new_width = $width;$new_height = $height;$offsetX = 0;$offsetY = 0
if ($crop) {
